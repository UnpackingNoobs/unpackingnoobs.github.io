...
They all seem to be addresses in the 0x76A... memory range. Highlight the next 4 bytes in the dump (for me it's _A0 0B AE 76_) and do a rightclick and then _Follow DWORD in Disassembler_. You should land in the kernel32.dll module at address 0x76AE0BA0 at the start of the _GetModuleFileNameW_ function.
...
FUNCPTR_t functions[NUM_FUNCS];

int main()
{
    // 'functions' gets initialized by someone else, similar to:
    // functions[0] = &func0;
    // functions[1] = &func1;
    // functions[2] = &func2;

    /* Indirect Call function 2 */
    functions[ID_FUNC_2]();
...
Do you remember how we loaded the _original.exe_ in PE-bear earlier? If not, do it again and have a look at the imports. What do you notice? There are much more imports than just 4 - to be precise, PE-bear tells us that 73 functions are imported from KERNEL32.dll.
...
* If a program needs to call external functions it does this via the IAT
